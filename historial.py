...
def simular_paciente(paciente):
    ...
    fecha_ultimo_lab = fecha_inicio  # fecha real del último laboratorio
    num_visita_total = 0
    ...
    for anio in range(1, 11):
        ...
        for i_vis, dia in enumerate(dias_anio):
            ...
            # --- HbA1c — solo cada 90 días usando fecha REAL de la visita ---
            # Usamos la fecha real en lugar de un contador fijo
            # porque las visitas son irregulares (random.sample)
            dias_desde_ultimo_lab = (fecha - fecha_ultimo_lab).days
            es_lab = dias_desde_ultimo_lab >= FREQ_LAB_DIAS
            if es_lab:
                hba1c = ar1(hba1c, 'hba1c', estado_dm, deriva_visita['hba1c'])
                fecha_ultimo_lab = fecha  # actualizar fecha real del último lab
...
if __name__ == "__main__":
    generar_historial()# historial.py
# Ejecutar desde la raiz del proyecto: python historial.py
# Genera: output/historial_medico.csv y output/historial_medico.xlsx
"""
Generador de Historial Médico — Modelo AR(1)
